...
/// Adapter for Cursor installations.
...
impl CursorAdapter {
    ...
    const TOOL_KEY: &'static str = "cursor";
}
...
impl ToolAdapter for CursorAdapter {
    ...
    fn detect_installation(&self) -> Result<Option<PathBuf>, HaalError> {
        ...
        for path in &candidates {
            debug!("Checking Cursor installation at: {}", path.display());
            if path.exists() {
                debug!("Found Cursor installation at: {}", path.display());
                return Ok(Some(path.clone()));
            }
        }

        debug!("No Cursor installation found");
        Ok(None)
    }
    ...
    fn parse_manifest(&self, content: &str) -> Result<Manifest, HaalError> {
        serde_json::from_str(content).map_err(|e| {
            HaalError::Validation(ValidationError {
                message: format!("Failed to parse Cursor manifest: {e}"),
                field: None,
            })
        })
    }
