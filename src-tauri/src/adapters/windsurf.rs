...
/// Adapter for Windsurf installations.
...
impl WindsurfAdapter {
    ...
    const TOOL_KEY: &'static str = "windsurf";
}
...
impl ToolAdapter for WindsurfAdapter {
    ...
    fn detect_installation(&self) -> Result<Option<PathBuf>, HaalError> {
        ...
        let candidates: Vec<PathBuf> = if cfg!(target_os = "windows") {
            ...
            // Linux
            vec![
                home.join(".config").join("Windsurf"),
                home.join(".windsurf"),
            ]
        };

        for path in &candidates {
            debug!("Checking Windsurf installation at: {}", path.display());
            if path.exists() {
                debug!("Found Windsurf installation at: {}", path.display());
                return Ok(Some(path.clone()));
            }
        }

        debug!("No Windsurf installation found");
        Ok(None)
    }
    ...
    fn default_destinations(&self) -> Vec<Destination> {
        ...
        if let Some(home) = dirs::home_dir() {
            dests.push(Destination {
                ...
                path: home.join(".windsurf").join("extensions"),
                enabled: true,
            });
        }
        dests
    ...
    fn parse_manifest(&self, content: &str) -> Result<Manifest, HaalError> {
        serde_json::from_str(content).map_err(|e| {
            HaalError::Validation(ValidationError {
                message: format!("Failed to parse Windsurf manifest: {e}"),
                field: None,
            })
        })
    }
