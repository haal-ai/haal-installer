...
/// Adapter for Kiro installations.
...
impl KiroAdapter {
    ...
    const TOOL_KEY: &'static str = "kiro";
}
...
impl ToolAdapter for KiroAdapter {
    ...
    fn detect_installation(&self) -> Result<Option<PathBuf>, HaalError> {
        ...
        let path = home.join(".kiro");
        debug!("Checking Kiro installation at: {}", path.display());

        if path.exists() {
            debug!("Found Kiro installation at: {}", path.display());
            return Ok(Some(path));
        }

        debug!("No Kiro installation found");
        Ok(None)
    }
    ...
    fn default_destinations(&self) -> Vec<Destination> {
        ...
        // Kiro also installs to .agents/skills (project-local)
        dests.push(Destination {
            tool_name: self.tool_name().to_string(),
    ...
    fn parse_manifest(&self, content: &str) -> Result<Manifest, HaalError> {
        serde_json::from_str(content).map_err(|e| {
            HaalError::Validation(ValidationError {
                message: format!("Failed to parse Kiro manifest: {e}"),
                field: None,
            })
        })
    }
